...
public class aticleBoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		aticleBoardDAO dao = new aticleBoardDAO();
...
// list		
		if(url.contains("list.do")) {
			...
			List<aticleBoardDTO> list = dao.list(start, end);
...
// view			
			...
			aticleBoardDTO dto = dao.view(num);
...
// insertReply			
			...
			String page = request.getContextPath()+"/aticle_servlet/list.do";
...
// pwdCheck		
			...
			if(result != null) {
				...
				page = request.getContextPath()+"/aticle_servlet/view.jsp";
				response.sendRedirect(page);
			}
			...
			aticleBoardDTO dto = new aticleBoardDTO();
...
// delete		
			...
			if(result!=null) {
				...
				String page = "${path}/aticle_servlet/list.do";
				response.sendRedirect(page);
...
// search
			...
			List<aticleBoardDTO> list = dao.search(option, keyword);
...
// comment_add
			...
			aticleBoardCommentDTO dto = new aticleBoardCommentDTO();
			dto.setWriter(writer);
			dto.setContent(content);
...
// comment_reply		
			...
			int board_num = Integer.parseInt(request.getParameter("board_num"));
			List<aticleBoardCommentDTO> list = dao.commentList(board_num);
			request.setAttribute("list", list);
